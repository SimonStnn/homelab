...
# # Load environment variable defaults from project .env when available
# if [ -f "/root/homelab/.env" ]; then
#   # shellcheck source=/root/homelab/.env
#   source "/root/homelab/.env"
# fi

# # Ensure volume directories have correct ownership for containers running as non-root
# VOLUME_DIR="${HOMELAB_VOLUMES:-/mnt/sdcard/homelab-volumes}"
